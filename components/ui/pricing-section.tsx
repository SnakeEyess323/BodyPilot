      ...
      <div className="w-full max-w-5xl mx-auto">
        <div className="flex flex-col items-center gap-4 mb-12">
          ...
          <div className="inline-flex items-center p-1.5 bg-white dark:bg-zinc-800/50 rounded-full border border-zinc-200 dark:border-zinc-700 shadow-sm">
            {["Aylık", "Yıllık"].map((period) => (
              <button
                key={period}
                onClick={() => setIsYearly(period === "Yıllık")}
                className={cn(
                  "px-8 py-2.5 text-sm font-medium rounded-full transition-all duration-300",
                  (period === "Yıllık") === isYearly
              ...
              >
                {period}
                {period === "Yıllık" && (
                  <span className="ml-1 text-xs text-violet-500 dark:text-violet-400">-20%</span>
                )}
              </button>
            ))}
          </div>
